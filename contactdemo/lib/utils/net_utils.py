...
def load_model(net,
    ...
    if not resume:
        if os.path.exists(model_dir):
            print(colored(f'model dir already exists: {model_dir}', 'red'))
            # assert False, 'model dir exists'
...
def load_network(net, model_dir, resume=True, epoch=-1, strict=True):
    ...
    if not os.path.exists(model_dir):
        print(colored(f'pretrained model does not exist: {model_dir}', 'red'))
        return 0
    ...
    print(colored('load model: {}'.format(model_path), 'green'))
